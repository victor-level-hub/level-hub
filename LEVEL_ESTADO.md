...
> **Última atualização:** 3 Jun 2026 — fecho do marco v2.7.7 (campo Plataforma editável)
...
3. ~~**Input de Platform**~~ — **FEITO na v2.7.7.** Select PS5/PC/Xbox no modal Configurações ▸ Operador, lê e grava em `player.platform`. Item removido da fila.
...
## 6. ESTADO DE DEPLOY (v2.7.7 — pronto, não commitado)

- [x] `index.html` (v2.7.0→v2.7.6) commitado e no ar — feito 3/Jun/2026
- [ ] **`index.html` (v2.7.7) commitar** no repo `level-hub`, branch main
  - Title commit (<50 chars): `feat: campo Plataforma editável (v2.7.7)`
  - Corpo do commit:
    ```
    Adicionado select PS5/PC/Xbox no modal Editar Status do Operador
    (Configurações · Operador). O estado player.platform já existia
    desde sempre (default 'PS5') e já era exibido no card de Operador,
    mas não tinha input.

    Análise IA da v2.7.6 já estava preparada para consumir o valor —
    agora chega real ao payload (player_profile.platform) em vez do
    fallback hardcoded.

    Layout do modal: a linha Prestige+Level virou grid 3-col
    Prestige+Level+Plataforma.

    LEVEL_ESTADO.md atualizado pra v2.7.7.
    ```
...
- Netlify auto-deploya em ~1min. Conferir em `le-vel.games` que o footer mostra v2.7.7.
